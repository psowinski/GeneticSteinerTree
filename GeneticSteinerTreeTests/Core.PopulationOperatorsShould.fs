...
let ``Cross population if it hits the probability`` () = 
   ...
   let corssPopulation = Population.createCross (fun _ _ _ -> [genotypeAbc])
...
let ``Not cross population if its out of the probability`` () = 
   ...
   let corssPopulation = Population.createCross (fun _ _ _ -> [])
...
let ``Mutate population if it hits the probability`` () = 
   ...
   let genotypeAbc = Genotype [Active "abc"]
   let mutatePopulation = Population.createMutate (fun _ _ -> genotypeAbc)
...
let ``Not mutate population if its out of the probability`` () = 
   ...
   let mutatePopulation = Population.createMutate (fun _ _ -> Genotype [])
